...
```{r}
gapminder |>
  mutate(lifeExp = if_else(condition = country == "Canada" & year == 1952, true = 70, false = lifeExp))
```
...
group_by(year) |> 
mutate(
meanLifeexp = mean(lifeExp), lifeExp_center = lifeExp - mean(lifeExp, na.rm = TRUE)) |>
ungroup()
```
